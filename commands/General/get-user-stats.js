...
const { getUser, getUserGames, searchUser, getAppOverviews, getUserBadges } = require('../../fetch-api.js')
...
async function createEmbed(userRaw, gamesRaw, pricesRaw, badgesRaw) {
    ...
    // User badges
    // FIXME: Error searching up certain users that have private badges
    if (badgesRaw) {
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (typeof(parseInt(searchInput)) === "number" && searchInput.length == 17) {
            ...
            if (games) {
                ...
                const prices = await getAppOverviews(ids)
                embed = await createEmbed(userRaw, games, prices, badges)
            } else {
                embed = await createEmbed(userRaw)
            }
        ...
        if (usersRaw.length == 1) {
            ...
            embed = await createEmbed(userRaw, games)
            await interaction.editReply( { content: '', embeds: [embed], components: [] })
            ...
            const message = await interaction.editReply({ content: `Is this the right user?`, embeds: [embed], components: [actionRow]})
            ...
            collector.on('collect', async i => {
                await i.deferUpdate()
                //TODO: Add ephemeral reply to non interaction users
